...
public abstract class AbstractCrudFormFactory<T> implements CrudFormFactory<T> {
	 ...
	 */
	private static final long serialVersionUID = 7372380722157431744L;
    ...
    @Override
    public void setRequiredProperties(CrudOperation operation, String... properties) {
    	getConfiguration(operation).setRequiredProperties(Arrays.asList(properties));
    }
    
    @Override
    public void setRequiredProperties(String... properties) {
    	Arrays.stream(CrudOperation.values()).forEach(operation -> setRequiredProperties(operation, properties));
    }
